local S = core.get_translator("atl_server_statistics")
...
core.register_chatcommand("stats", {
    ...
    func = function(player_name, param)
        ...
        if not atl_server_statistics.player_has_stats(target_player_name) then
            return core.chat_send_player(player_name, core.colorize(atl_server_statistics.color_message, S("-!- No statistics available for ") .. target_player_name))
        end
        ...
        core.chat_send_player(player_name, core.colorize(atl_server_statistics.color_message, stats_message))
    end,
})
    ...
    func = function(player_name)
        ...
        if atl_server_statistics.reset_requests[player_name] then
            if current_time - atl_server_statistics.reset_requests[player_name] <= atl_server_statistics.time_before_end_request then
                ...
                core.chat_send_player(player_name, core.colorize(atl_server_statistics.reset_color_message, S("-!- Your statistics have been reset.")))
                atl_server_statistics.reset_requests[player_name] = nil
            else
                atl_server_statistics.reset_requests[player_name] = current_time
                return core.chat_send_player(player_name, core.colorize(atl_server_statistics.reset_color_message, S("-!- Statistics reset request has expired. Please try again.")))
            end
        else
            atl_server_statistics.reset_requests[player_name] = current_time
            core.chat_send_player(player_name, core.colorize(atl_server_statistics.reset_color_message, S("-!- To confirm the reset of your statistics, type /reset again within the next ") .. atl_server_statistics.time_before_end_request .. S(" seconds.")))
        end
    end,
})
...
if core.settings:get_bool("atl_server_statistics.simplified_command") ~= true then
    core.register_chatcommand("s", core.registered_chatcommands["stats"])
    core.register_chatcommand("ld", core.registered_chatcommands["leaderboard"])
end
